...
// real, isolated Postgres to run against — per the migration plan's "the
// database is the thing most likely to differ, so faking it defeats the
// purpose" reasoning.
//
// Every Setup call probes for the migrated schema and (re)applies
// migrations when it is missing or stale — deliberately NOT a sync.Once:
// other tests in the same binary (internal/db's own migrate_test.go) drop
// and rebuild the public schema, and nothing in the Go toolchain guarantees
// which test file runs first. The probe makes rig correctness independent
// of test ordering. Each individual test then gets a clean slate via a
// dynamic TRUNCATE of every public table (except goose_db_version) before
// it starts, so unrelated tests never see each other's rows. Because
// truncation is process-wide state, tests using this rig must run serially:
// `go test -p 1 ./...`.
package testrig
...
// docker-compose.test.yml Postgres, overridable for CI.
...
// domain table so the test starts from an empty database, re-seeds the
// app_installation singleton that truncation just removed, opens a pool for
...
func Setup(t *testing.T) *Rig {
	...
	q := gen.New(pool)
	// Mi Casa has no tombstone table: goose's Up migration seeds
	// app_installation row 1 as 'pending', but truncateAll just removed it
	// along with every other table. EnsureInstallation puts it straight
	// back so every test starts with the singleton row present, matching a
	// freshly migrated database rather than an empty one.
	if err := q.EnsureInstallation(ctx); err != nil {
		t.Fatalf("testrig: ensure installation: %v", err)
	}
...
// (one lookup) and, unlike a once-per-binary guard, immune to another test
// in the same binary dropping the schema after we migrated it.
//
// It compares goose_db_version against the highest embedded migration
// rather than checking that some table exists: a table probe only ever
// answers "migration 1 ran", so the first developer to add a second
// migration gets a confusing failure in an unrelated package instead of a
// migrated database.
